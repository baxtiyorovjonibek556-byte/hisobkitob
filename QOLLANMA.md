...
Yangi versiya berilsa: repoda `index.html` va `sw.js` ni almashtirasiz — bo'ldi, ilova **keyingi ochilishda o'zi yangilanadi** (bir marta avtomatik qayta yuklanadi). Qaysi versiya ishlayotganini tekshirish: **Sozlamalar oxiridagi «Hisob · vXX»** yozuvi.

Juda o'jarlik qilsa: manzil oxiriga `?fresh=1` qo'shib bir marta oching (masalan `…/hisob/?fresh=1`) — kesh tozalanadi, **yozuvlaringizga tegilmaydi**.

⚠️ Brauzer sozlamalaridan «Sayt ma'lumotlarini tozalash / Clear site data»ni **bosmang** — u hisob yozuvlarini ham o'chirib yuboradi (faqat JSON zaxira yoki Firebase bo'lsa tiklanadi).
